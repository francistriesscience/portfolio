...
function parseColor(color: string): { rgb: number[]; alpha?: number } {
...
export function SpecialEffectParticles({
  className,
  quantity = 100,
  staticity = 50,
  ease = 50,
  ...
  const colorData = React.useMemo(() => parseColor(color), [color])
  const rgb = colorData.rgb
  const baseAlpha = colorData.alpha
  ...
  const circleParams = React.useCallback((): Circle => {
    ...
    const alpha = 0
    const targetAlpha = parseFloat((Math.random() * 0.6 + 0.1).toFixed(1)) * (baseAlpha ?? 1)
